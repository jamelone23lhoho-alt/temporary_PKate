...
export default function ExportPage() {
  ...
  const handleMultiPhotoUpload = async (field, files) => {
    ...
    setBoxPhotos(prev => ({
      ...prev,
      [field]: [...(Array.isArray(prev[field]) ? prev[field] : prev[field] ? [prev[field]] : []), ...urls]
    }));
  };
  ...
  const removePhoto = (field, idx) => {
    setBoxPhotos(prev => ({
      ...prev,
      [field]: (Array.isArray(prev[field]) ? prev[field] : [prev[field]]).filter((_, i) => i !== idx)
    }));
  };
  ...
  const openExportForm = async (exp) => {
    ...
    if (existing) {
      ...
      setEfForm({ ...base, weight_result: existing.weight_result || wr, weight_diff: existing.weight_diff || '', price_per_kg: existing.price_per_kg || '', price_per_diff: existing.price_per_diff || '', total_thb: existing.total_thb || '', total_mnt: existing.total_mnt || '', type_box: existing.type_box || 'Personal' });
    } else {
      setEfEditing(null);
      setEfForm({ ...base, weight_result: wr, weight_diff: '', price_per_kg: '', price_per_diff: '', total_thb: '', total_mnt: '', type_box: 'Personal' });
    }
    setSaving(false);
    setDetailOpen(false);
    setEfModalOpen(true);
  ...
  const updateEfField = (key, val) => {
    ...
    if (['price_per_kg','weight_result','price_per_diff','weight_diff'].includes(key)) nf.total_thb = calcEfTotal(nf);
    setEfForm(nf);
  };
    ...
    <AppShell>
      ...
      <div className="fade-in">
        <div className="flex items-center justify-between mb-6">
          ...
          {hasPermission(role, 'export_add') && (
            <button onClick={openAdd} className="px-5 py-2.5 rounded-lg text-sm font-semibold text-white transition-all hover:-translate-y-0.5" style={{ background: 'var(--black)' }}>+ Add Export</button>
          )}
        </div>
        <div className="relative mb-5">
          ...
          <input type="text" placeholder="Search tracking, client..." value={search} onChange={(e) => setSearch(e.target.value)} className={`${inputCls} pl-11`} style={{ ...inputStyle, background: 'var(--white)' }} />
        </div>

        {loading ? (
          <div className="flex flex-col items-center py-16 gap-3" style={{ color: 'var(--text-muted)' }}><div className="spinner" /><span className="text-sm">Loading...</span></div>
        ) : filtered.length === 0 ? (
          <div className="text-center py-16" style={{ color: 'var(--text-muted)' }}><span className="material-icons-outlined block mb-3" style={{ fontSize: 48, color: 'var(--grey)' }}>inventory_2</span><p>No exports found</p></div>
        ) : (
      ...
      <Modal isOpen={detailOpen} onClose={() => setDetailOpen(false)} title="Export Detail" footer={
        ...
        {current && (<div>
          ...
          <div className="mt-3"><div className="text-xs uppercase tracking-wide mb-0.5" style={{ color: 'var(--text-muted)' }}>Remark</div><div className="text-sm">{current.remark || '-'}</div></div>
      ...
      <Modal isOpen={boxDetailOpen} onClose={() => setBoxDetailOpen(false)} title="Box Detail">
        {currentBox && (<div>
          ...
          {currentBox.items && currentBox.items.item && (<div className="mt-4 pt-3" style={{ borderTop: '1px solid var(--border)' }}>
            <div className="text-sm font-bold mb-2">Item</div>
            <div className="text-sm" style={{ color: 'var(--text-secondary)' }}>{currentBox.items.item} — {currentBox.items.unit} unit(s) — {currentBox.items.type}</div>
          </div>)}
          {Object.keys(currentBox.photos || {}).some(k => { const v = currentBox.photos[k]; return Array.isArray(v) ? v.length > 0 : !!v; }) && (
              ...
              <div className="grid grid-cols-2 gap-2">
                {Object.entries(currentBox.photos).map(([k, v]) => {
                  ...
                  return urls.map((url, i) => (
                    <div key={`${k}-${i}`}><div className="text-xs mb-1" style={{ color: 'var(--text-muted)' }}>{PHOTO_LABELS[k]}</div><img src={url} className="w-full h-24 object-cover rounded-lg" alt="" /></div>
                  ));
                })}
              </div>
            </div>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Export' : 'Add Export'} footer={<>
        ...
        <F label="Client *">
          {role === 'Origin Officer' ? <input value="CTL000 (Unknown)" readOnly className={inputCls} style={roStyle} /> : (
        ...
        <F label="Payment"><select value={form.payment} onChange={(e) => setForm({...form, payment: e.target.value})} className={inputCls} style={inputStyle}><option value="No">No</option><option value="Yes">Yes</option></select></F>
        <F label="Box Type"><select value={form.box_type} onChange={(e) => setForm({...form, box_type: e.target.value})} className={inputCls} style={inputStyle}><option value="">-- Select --</option>{BOX_TYPES.map(t => <option key={t} value={t}>{t}</option>)}</select></F>
      ...
      <Modal isOpen={boxModalOpen} onClose={() => setBoxModalOpen(false)} title={editingBox ? 'Edit Box' : 'Add Box'} footer={<>
        ...
        <div className="p-3 rounded-lg mb-2" style={{ background: 'var(--cream)', border: '1px solid var(--border)' }}>
          <div className="flex gap-2 mb-2">
            <input placeholder="Item name" value={boxItems.item || ''} onChange={(e) => setBoxItems({...boxItems, item: e.target.value})} className={`${inputCls} flex-1`} style={inputStyle} />
            <input placeholder="Unit" type="number" value={boxItems.unit || ''} onChange={(e) => setBoxItems({...boxItems, unit: e.target.value})} className={inputCls} style={{ ...inputStyle, width: 70 }} />
          </div>
          <select value={boxItems.type || ''} onChange={(e) => setBoxItems({...boxItems, type: e.target.value})} className={inputCls} style={inputStyle}>
              ...
              <div className="flex flex-wrap gap-2">
                {photos.map((url, i) => (
                  <div key={i} className="relative">
                    <img src={url} className="h-16 w-16 object-cover rounded-lg" alt="" />
                    <button onClick={() => removePhoto(field, i)} className="absolute -top-1 -right-1 w-5 h-5 rounded-full text-white text-xs flex items-center justify-center" style={{ background: 'var(--danger)' }}>×</button>
                  </div>
                ))}
                <div onClick={() => document.getElementById(`bp-${field}`).click()} className="w-16 h-16 border-2 border-dashed rounded-lg flex items-center justify-center cursor-pointer" style={{ borderColor: 'var(--border)', color: 'var(--grey)' }}>
                  <span className="material-icons-outlined" style={{ fontSize: 20 }}>add_photo_alternate</span>
                </div>
              </div>
      ...
      <Modal isOpen={efModalOpen} onClose={() => setEfModalOpen(false)} title={efEditing ? 'Edit Export Form' : 'New Export Form'} footer={<>
        ...
        <div className="grid grid-cols-2 gap-3 mb-2">
          <F label="Export Date"><input value={fmtD(efForm.export_date)} readOnly className={inputCls} style={roStyle} /></F>
          <F label="Order Code"><input value={efForm.order_code || ''} readOnly className={inputCls} style={roStyle} /></F>
        </div>
        <F label="Client"><input value={efForm.client || ''} readOnly className={inputCls} style={roStyle} /></F>
        <div className="grid grid-cols-3 gap-3">
          <F label="Total Boxes"><input value={efForm.total_boxes || 0} readOnly className={inputCls} style={roStyle} /></F>
          <F label="Total GW"><input value={efForm.total_gw || 0} readOnly className={inputCls} style={roStyle} /></F>
          <F label="Weight Result"><input value={efForm.weight_result || 0} readOnly className={inputCls} style={roStyle} /></F>
        </div>
        ...
        <F label="Type Box"><select value={efForm.type_box} onChange={(e) => setEfForm({...efForm, type_box: e.target.value})} className={inputCls} style={inputStyle}>{['Personal','Special1','Special2','Special3'].map(t => <option key={t} value={t}>{t}</option>)}</select></F>
      </Modal>
