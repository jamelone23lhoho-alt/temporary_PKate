...
export default function ExportFormPage() {
  ...
  const selectExport = async (exp) => {
    ...
    setEditing(null);
    setForm({ export_id: exp.id, export_date: exp.export_date || '', order_code: exp.order_code || '', client: exp.client || '', total_boxes: exp.total_boxs || 0, total_gw: exp.total_gw || 0, weight_result: wr, weight_diff: '', price_per_kg: '', price_per_diff: '', total_thb: '', total_mnt: '', type_box: 'Personal' });
    setSaving(false);
    setModalOpen(true);
  };
  ...
  const openEdit = (row) => {
    setEditing(row);
    setForm({ export_id: row.export_id || '', export_date: row.export_date || '', order_code: row.order_code || '', client: row.client || '', total_boxes: row.total_boxes || 0, total_gw: row.total_gw || 0, weight_result: row.weight_result || 0, weight_diff: row.weight_diff || '', price_per_kg: row.price_per_kg || '', price_per_diff: row.price_per_diff || '', total_thb: row.total_thb || '', total_mnt: row.total_mnt || '', type_box: row.type_box || 'Personal' });
  ...
  return (
    <AppShell>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Export Form' : 'New Export Form'} footer={<>
        ...
        <F label="Type Box"><select value={form.type_box} onChange={(e) => setForm({...form, type_box: e.target.value})} className={inputCls} style={inputStyle}>{TYPE_BOXES.map(t => <option key={t} value={t}>{t}</option>)}</select></F>
      </Modal>
